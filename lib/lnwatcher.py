...
class LNChanCloseHandler(PrintError):
    ...
    def find_and_sweep_our_ctx_to_local(self, ctx, our_pcp: bytes):
        ...
        for output_idx, (type, addr, val) in enumerate(ctx.outputs()):
            ...
            return
        # TODO if we the CSV lock is still pending, this will fail
        sweep_tx = self.create_sweeptx_ctx_to_local(ctx, output_idx, witness_script, our_localdelayed_privkey, False)
        self.network.broadcast_transaction(sweep_tx,
