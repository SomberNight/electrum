...
CONTRIB_APPIMAGE="$CONTRIB/build-linux/appimage_arm"
...
QT5_VERSION="1deea3debce6bfd0fd45545b3a8e1de80380a9a5"  # tag v5.11.3


VERSION=`git describe --tags --dirty --always`
APPIMAGE="$DISTDIR/electrum-$VERSION-arm32v7.AppImage"
...
download_if_not_exist "$CACHEDIR/appimagetool" "https://github.com/AppImage/AppImageKit/releases/download/12/appimagetool-armhf.AppImage"
verify_hash "$CACHEDIR/appimagetool" "8bca91ae4dac9a6505a4939f29ce5c75b74a8c8e80d56f738b17863fbd6d2770"
...
# ref https://gist.github.com/elpaso/8ca33ddca336b7ecf0c25c77c7367572

info "building qt5."
(
    git clone https://code.qt.io/qt/qt5.git "$CACHEDIR"/qt5 \
        || (cd "$CACHEDIR"/qt5 && git reset --hard && git pull)
    cd "$CACHEDIR"/qt5
    git reset --hard "$QT5_VERSION"
    git clean -f -x -q
    ...
    TZ=UTC faketime -f '2019-01-01 01:01:01' make -j4
    make -s install
)
...
appdir_python() {
  env \
    PYTHONNOUSERSITE=1 \
    LD_LIBRARY_PATH="$APPDIR/usr/lib:$APPDIR/usr/lib/arm-linux-gnueabihf${LD_LIBRARY_PATH+:$LD_LIBRARY_PATH}" \
...
info "Copying libusb"
cp -f /usr/lib/arm-linux-gnueabihf/libusb-1.0.so "$APPDIR/usr/lib/libusb-1.0.so" || fail "Could not copy libusb"
...
(
    ...
    env VERSION="$VERSION" ARCH=armhf ./squashfs-root/AppRun --no-appstream --verbose "$APPDIR" "$APPIMAGE"
)
