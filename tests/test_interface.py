...
from .toy_server import ToyServer, ToyServerSession
...
class TestInterface(ElectrumTestCase):
    ...
    async def asyncSetUp(self):
        ...
        self._toy_server = ToyServer()
        await self._toy_server.start()
        self.network = MockNetwork(config=self.config)
        for _ in range(10):  # mine some blocks
            await self._toy_server.mine_block()
        await self._toy_server.set_up_faucet(config=self.config)

    async def asyncTearDown(self):
        if self.network.interface:
            await self.network.interface.close()
        await self._toy_server.stop()
        await super().asyncTearDown()

    async def _start_iface_and_wait_for_sync(self):
        interface = Interface(network=self.network, server=ServerAddr(host="127.0.0.1", port=self._toy_server.server_port, protocol="t"))
    ...
    def _get_server_session(self) -> ToyServerSession:
        return self._toy_server.get_session_by_name("alice")

    async def test_client_syncs_headers_to_tip(self):
        interface = await self._start_iface_and_wait_for_sync()
        self.assertEqual(self._toy_server.cur_height, interface.tip)
        self.assertFalse(interface.got_disconnected.is_set())

    async def test_transaction_get(self):
        interface = await self._start_iface_and_wait_for_sync()
        # inject a tx into the server:
        self._toy_server._add_tx(Transaction(""))
        ...
        self.assertEqual(rawtx, self._toy_server.txs["bdae818ad3c1f261317738ae9284159bf54874356f186dbc7afd631dc1527fcb"].hex())
    ...
    async def test_transaction_broadcast(self):
        ...
        self.assertEqual(bfh(rawtx1), self._toy_server.txs.get(tx.txid()))
        # now request tx.
    ...
    async def test_dont_request_gethistory_if_status_change_results_from_mempool_txs_simply_getting_mined(self):
        ...
        funding_tx = await self._toy_server.ask_faucet([TxOutput.from_address_and_value(w1_addr, 1 * COIN)])
        ...
        self.assertEqual(
            ...
            {funding_txid: 0})
        # mine funding tx
        await self._toy_server.mine_block(txs=[funding_tx])
