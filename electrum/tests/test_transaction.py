...
class TestLegacyPartialTxFormat(TestCaseForTestnet):
    ...
    def test_trustedcoin_segwit_2fa_psbt_to_legacy_partial_tx(self):
        ...
        import copy
        orig_deepcopy_dict = copy._deepcopy_dict
        try:
            def c(*args, **kwargs):
                print(f"copy._deepcopy_dict() called with x={args[0]}")
                return orig_deepcopy_dict(*args, **kwargs)

            copy._deepcopy_dict = c
            copy._deepcopy_dispatch[dict] = copy._deepcopy_dict

            raw_tx = serialize_tx_in_legacy_format(tx, wallet=wallet)
        finally:
            copy._deepcopy_dict = orig_deepcopy_dict
            copy._deepcopy_dispatch[dict] = copy._deepcopy_dict
