...
class Jade_Client(HardwareClientBase):
    ...
    def __init__(self, device: str, plugin: HW_PluginBase):
        ...
        # Initial connection is with short timeout (to test availability)
        self.jade = JadeAPI.create_serial(device, timeout=1)
        self.jade.connect()

        verinfo = self.jade.get_version_info()
        self.fwversion = semver.VersionInfo.parse(verinfo['JADE_VERSION'])
        self.efusemac = verinfo['EFUSEMAC']

        # Reconnecting with default timeout (for all subsequent calls)
...
class JadePlugin(HW_PluginBase):
    ...
    def create_client(self, device, handler):
        ...
        if self.MIN_SUPPORTED_FW_VERSION > client.fwversion.finalize_version():
