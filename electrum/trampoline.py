...
def is_hardcoded_trampoline(node_id: bytes):
...
def create_trampoline_route(
    ...
    if (invoice_features.supports(LnFeatures.OPTION_TRAMPOLINE_ROUTING_OPT)
        ...
        if not r_tags:  # presumably the recipient has public channels
            ...
            #   channel layours where two TFs are needed for a payment to succeed, e.g. both
            #   endpoint connected to T1 and T2, and sender only has send-capacity with T1, while
            ...
            is_legacy = not is_hardcoded_trampoline(pubkey)  # TODO
...
def create_trampoline_onion(*, route, amount_msat, final_cltv, total_msat, payment_hash, payment_secret):
    ...
    for i in range(num_hops):
        ...
        # legacy
        if i == num_hops - 2 and route_edge.invoice_features:  # TODO
