...
def make_htlc_tx(*, cltv_expiry: int, inputs: List[PartialTxInput], output: PartialTxOutput) -> PartialTransaction:  # FIXME xxxxx
...
def make_received_htlc(
    *,
    ...
    assert type(cltv_expiry_abs) is int
    ...
    *,
    ...
    if is_received_htlc:
        return make_received_htlc(revocation_pubkey=remote_revocation_pubkey,
                                  ...
                                  cltv_expiry_abs=cltv_expiry_abs)
    else:
...
def possible_output_idxs_of_htlc_in_ctx(*, chan: 'Channel', pcp: bytes, subject: 'HTLCOwner',
    ...
    amount_msat, cltv_expiry, payment_hash = htlc.amount_msat, htlc.cltv_abs, htlc.payment_hash
...
class UpdateAddHtlc:
    ...
    cltv_abs = attr.ib(type=int, kw_only=True)  # FIXME xxxxx
