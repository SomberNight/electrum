...
# TODO 150, 450, 600
SPOT_RATE_REFRESH_TARGET = 20      # approx. every 2.5 minutes, try to refresh spot price
SPOT_RATE_CLOSE_TO_STALE = 120
SPOT_RATE_EXPIRY = 240              # spot price becomes stale after 10 minutes
...
class FxThread(ThreadJob, EventListener, NetworkRetryManager[str]):
    ...
    async def run(self):
        while True:
            ...
            if self._can_retry_addr("spot", urgent=is_urgent):
                self._trying_addr_now("spot")
                # refresh spot price
    ...
    def on_quotes(self, *, received_new_data: bool = False):
        if received_new_data:
            self._on_connection_successfully_established("spot")
