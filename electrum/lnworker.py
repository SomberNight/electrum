...
class LNWallet(LNWorker):
    ...
    def __init__(self, wallet: 'Abstract_Wallet', xprv):
        ...
        self.sent_buckets = dict()                    # payment_bkey -> (amount_sent, amount_failed)
        self.received_mpp_htlcs = dict()              # type: Dict[bytes, ReceivedMPPStatus]  # payment_bkey -> ReceivedMPPStatus
    ...
    async def pay_to_route(
        ...
        payment_bkey = payment_hash + payment_secret
        # if we sent MPP to a trampoline, add item to sent_buckets
        if self.uses_trampoline() and amount_msat != total_msat:
            if payment_bkey not in self.sent_buckets:
                self.sent_buckets[payment_bkey] = (0, 0)
    ...
    def _get_payment_bkey(self, payment_hash: bytes) -> bytes:
    ...
    def check_mpp_status(
        ...
        self.update_mpp_with_received_htlc(
            payment_bkey=payment_bkey, scid=short_channel_id, htlc=htlc, expected_msat=expected_msat)
        mpp_resolution = self.received_mpp_htlcs[payment_bkey].resolution
        if mpp_resolution == RecvMPPResolution.WAITING:
            bundle = self.get_payment_bundle(payment_hash)
            if bundle:
                payment_bkeys = [self._get_payment_bkey(h) for h in bundle]
                if payment_bkey not in payment_bkeys:
                    # outer trampoline onion secret differs from inner onion
                    # the latter, not the former, might be part of a bundle
                    payment_bkeys = [payment_bkey]
            else:
                payment_bkeys = [payment_bkey]
            first_timestamp = min([self.get_first_timestamp_of_mpp(bkey) for bkey in payment_bkeys])
            ...
            elif all([self.is_mpp_amount_reached(bkey) for bkey in payment_bkeys]):
                mpp_resolution = RecvMPPResolution.ACCEPTED
            elif time.time() - first_timestamp > self.MPP_EXPIRY:
                mpp_resolution = RecvMPPResolution.EXPIRED

            if mpp_resolution != RecvMPPResolution.WAITING:
                for bkey in payment_bkeys:
                    if bkey in self.received_mpp_htlcs:
                        self.set_mpp_resolution(payment_bkey=bkey, resolution=mpp_resolution)

        self.maybe_cleanup_mpp_status(payment_bkey, short_channel_id, htlc)
    ...
    def update_mpp_with_received_htlc(
        ...
        *,
        ...
        # add new htlc to set
        mpp_status = self.received_mpp_htlcs.get(payment_bkey)
        ...
        if expected_msat != mpp_status.expected_msat:
            self.logger.info(
                f"marking received mpp as failed. inconsistent total_msats in bucket. {payment_bkey.hex()=}")
        ...
        self.received_mpp_htlcs[payment_bkey] = mpp_status

    def set_mpp_resolution(self, *, payment_bkey: bytes, resolution: RecvMPPResolution):
        mpp_status = self.received_mpp_htlcs[payment_bkey]
        self.received_mpp_htlcs[payment_bkey] = mpp_status._replace(resolution=resolution)

    def is_mpp_amount_reached(self, payment_bkey: bytes) -> bool:
        mpp_status = self.received_mpp_htlcs.get(payment_bkey)
        if not mpp_status:
            return False
    ...
    def get_first_timestamp_of_mpp(self, payment_bkey: bytes) -> int:
        mpp_status = self.received_mpp_htlcs.get(payment_bkey)
        if not mpp_status:
    ...
    def maybe_cleanup_mpp_status(
        ...
        if not mpp_status.htlc_set and payment_bkey in self.received_mpp_htlcs:
            self.received_mpp_htlcs.pop(payment_bkey)
